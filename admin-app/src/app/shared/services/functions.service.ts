...
export class FunctionService extends BaseService {
  ...
  getAll() {
    return this.http.get<Function[]>(`${environment.apiUrl}/api/functions`, { headers: this._sharedHeaders }).pipe(
      map((res: Function[]) => {
        return res;
      }),
  ...
  getAllByParentId(parentId) {
    ...
    return this.http.get<Function[]>(url, { headers: this._sharedHeaders }).pipe(
      map((res: Function[]) => {
        return res;
      }),
      catchError(this.handleError)
  ...
  getAllCommandsByFunctionId(functionId: string) {
    return this.http
      .get<Function[]>(`${environment.apiUrl}/api/functions/${functionId}/commands`, { headers: this._sharedHeaders })
      .pipe(
        map((res: Function[]) => {
          return res;
        }),
